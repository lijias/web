...
<div style="margin: 50px;">
    ...
    <input type="hidden" name="tags_str" id="tags_str"
           value="<?php echo isset($editing['tags_str']) ? $editing['tags_str'] : ''; ?>">
</div>
<script>
    var content_id = "<?php echo $editing['id']; ?>";

    /*
     *  频道
     */

    <?php if (isset($editing['channel_str']) && $editing['channel_str'] != '') {?>
    //    在编辑时, 需要把数据库中已有的标签,
    var channel_element = $('#channel').select2(); // 初始化
    var channel_request = $.ajax({
        url: site_url + '/select2/select2_4/get_obj?type=channel&content_id=' + content_id // 你的数据实际上来自哪里
    });
    channel_request.then(function (allData) {
//        这假设数据作为数据对象数组返回。
...
//            追加数据
            channel_element.append(option);
        }
//        更新需展示的数据
        channel_element.trigger('change');
    });
    <?php  }?>
...
//        更新需展示的数据
     ...
     *  标签
     */
    <?php if (isset($editing['tags_str']) && $editing['tags_str'] != '') {?>
    //    在编辑时, 需要把数据库中已有的标签,
    var tag_element = $('#tag').select2(); // 初始化
    var tag_request = $.ajax({
        url: site_url + '/select2/select2_4/get_obj?type=tag&content_id=' + content_id // 你的数据实际上来自哪里
    });
    tag_request.then(function (allData) {
//        这假设数据作为数据对象数组返回。
        var data = JSON.parse(allData);
        for (var d = 0; d < data.length; d++) {
            var item = data[d];
...
//        multiple: true 多选
        ...
        width: '85%',
        ajax: {
            url: site_url + '/select2/select2_4/getObj?type=tag',
            dataType: "json",
//            用户停止输入后多久开始搜索,单位毫秒
            delay: 1000,
//            点击时
...
//                    数据需已results为键值的数组
    ...
    $('#tag').on("select2:select", function (e) {
        ...
        if (e.params.data.tag) {
            var tag = e.params.data.tag;
        } else {
            var tag = e.params.data.text;
        }
        var new_tag = tags_str + ';' + tag;
        $('#tags_str').val(new_tag);
        var tag_arr = $('#tags_str').val().split(';');
        var tag_len = tag_arr.length;
//        把标签的值组装成规格的数组
        var data = new Array();
        for (var k = 0; k < tag_len; k++) {
//            生成json对象
            data[k] = {};
//        新的频道赋值到json对象中
            data[k].id = tag_arr[k];
            data[k].text = tag_arr[k];
        }
//        清除全部选择
        tag_element.val(null).trigger('change');
//        生成新的标签
