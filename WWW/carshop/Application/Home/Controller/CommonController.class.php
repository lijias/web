...
use Admin\Controller\CateController;
class CommonController extends Controller 
{
    ...
    public function getcate($cateid) 
    {
    	if ($cateid) 
    	{
    		...
    		if ($cates['class']==2) 
    		{
    			$topcates=$cate->where('id='.$cates['parentid'])->find();//顶级栏目
    		...
    		if ($cates['class']==3)
    		{
    			$upcates=$cate->where('id='.$cates['parentid'])->find();//三级栏目对应的二级栏目
    			$topcates=$cate->where('id='.$upcates['parentid'])->find();
    			$son3=$cate->where('parentid='.$upcates['id'])->select();
    			$son2=$cate->where('parentid='.$topcates['id'])->select();
    			$this->assign("son2",$son2);
    			$this->assign("son3",$son3);
    			$this->assign("topname",$topcates['name']);
    			$this->assign("son3pid",$upcates['id']);   
    		}
    	}
    }
    public function nav()
