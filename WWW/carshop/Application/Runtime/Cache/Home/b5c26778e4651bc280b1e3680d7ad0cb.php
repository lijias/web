	...
	<div class="nav">
    	<ul>
        	...
        	<?php if(is_array($navres)): foreach($navres as $key=>$v): ?><li><a href="/carshop/index.php/Home/<?php if($v['type'] == 1): ?>Arlist
            										...
            										<?php elseif($v['type'] == 7): ?>Job<?php endif; ?>/index/cateid/<?php echo ($v["id"]); ?>" title="关于我们"><?php echo ($v["name"]); ?></a></li><?php endforeach; endif; ?>
       </ul>
    </div>
    ...
    <div class="l_pro">
      <div class="l_pro_t"><?php echo ($topname); ?></div>
			...
			<?php foreach ($son3 as $k1=>$v1):?>
				<li><a style="padding-left:50px;" href="/carshop/index.php/Home/Search/index/cateid/<?php echo $v1['id'];?>" title="<?php echo $v1['name'];?>"><?php echo $v1['name'];?></a></li>			
			<?php endforeach;?>	
			<?php endif;?>								
			</ul>
			</li>
...
邮箱：这里是您公司的邮箱地址</div>
        ...
        <div class="fyRight">
      			<div class="title"> <span class="fl"> <?php echo ($selftop); ?></span> 
      			<span class="fr">当前位置：<a href="/carshop/index.php/" title="首页">首页</a> > 
      			<?php if(is_array($pres)): $i = 0; $__LIST__ = $pres;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i; if($i != count($pres)): ?><a href="/carshop/index.php/Home/<?php if($v['type'] == 1): ?>Arlist
            										<?php elseif($v['type'] == 2): ?>Page
            										<?php elseif($v['type'] == 3): ?>Message
            										<?php elseif($v['type'] == 4): ?>Zplist
            										<?php elseif($v['type'] == 5): ?>Carlist
            										<?php elseif($v['type'] == 6): ?>Rylist
            										<?php elseif($v['type'] == 7): ?>Job<?php endif; ?>/index/cateid/<?php echo ($v["id"]); ?>" ><?php echo ($v["name"]); ?></a> &gt;
      				<?php else: ?>
      				<font><?php echo ($v["name"]); ?></font><?php endif; endforeach; endif; else: echo "" ;endif; ?>
      			</span></div>
		 <ul class="news_list">
		 <?php if(is_array($list)): foreach($list as $key=>$v): ?><li><a href="http://0001543.ks.panguweb.cn/news_view.php?id=65" title="<?php echo ($v["title"]); ?>"><span class="fl"><?php echo ($v["title"]); ?></span><span class="fr"><?php echo (date("Y-m-d",$v["time"])); ?></span></a></li><?php endforeach; endif; ?>
		</ul>    
