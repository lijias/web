            ...
            <form name="myform" id="myform" method="post">
                <div class="result-title">
                    <div class="result-list">
                        <a href="/carshop/index.php/Admin/Role/add.html" class="btn btn-primary btn2"><i class="icon-font"></i>新增角色</a>
                        <a id="batchDel" href="javascript:void(0)"><i class="icon-font"></i><input form="myform" formaction="/carshop/index.php/Admin/Role/bdelete.html" type="submit" value="批量删除" class="btn btn-primary btn2"/></a>
                    </div>
                </div>
                <div class="result-content">
                    <table class="result-tab" width="100%">
                        ...
                        <?php if(is_array($rolres)): foreach($rolres as $key=>$v): ?><tr>
                        
                            <td class="tc">
                            <?php if($v['id'] != 1): ?><input name="ids[]" value="<?php echo ($v["id"]); ?>" class="check" type="checkbox"><?php endif; ?> 
                            </td>
                        
                            <td><?php echo ($v["id"]); ?></td>
                            <td><?php echo ($v["rolename"]); ?></td>
                            <?php if($v['pri_id_list'] == '*'): ?><td>所有权限</td>
                            <?php else: ?>
                            <td><?php echo ($v["pri_name"]); ?></td><?php endif; ?>
                            <td>
                                <a class="link-update" href="/carshop/index.php/Admin/Role/update/id/<?php echo ($v["id"]); ?>.html">修改</a>
                               <?php if($v['id'] != 1): ?><a class="link-del" onclick="return confirm('是否删除该角色吗!');" href="/carshop/index.php/Admin/Role/delete/id/<?php echo ($v["id"]); ?>.html">删除</a><?php endif; ?>
                            </td>
